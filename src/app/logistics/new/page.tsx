...
interface ShipmentDraft {
  selectedDeliveries: Array<[string, SelectedDelivery]>
  formData: {
    ...
    logistics_region: 'East' | 'Central' | 'West' | ''
    planned_departure_date: string
    planned_arrival_days: number
...
export default function NewShipmentPage() {
  ...
  const saveDraft = useCallback(() => {
    try {
      const draft: ShipmentDraft = {
        selectedDeliveries: Array.from(selectedDeliveries.entries()),
        formData,
        currentStep: step,
        warehouseType,
  ...
  }, [selectedDeliveries, formData, step, warehouseType])

  const loadDraft = useCallback((): ShipmentDraft | null => {
    try {
      const saved = localStorage.getItem(DRAFT_KEY)
      if (!saved) return null

      const draft: ShipmentDraft = JSON.parse(saved)
  ...
  const restoreDraft = useCallback(() => {
    ...
    // Restore selected deliveries
    const restoredMap = new Map<string, SelectedDelivery>(draft.selectedDeliveries)
    setSelectedDeliveries(restoredMap)
    ...
    setWarehouseType(draft.warehouseType)
    setDraftSavedAt(draft.savedAt)
  ...
  useEffect(() => {
    // Only auto-save if user has made changes (has selected deliveries or filled form)
    const hasChanges = selectedDeliveries.size > 0 ||
                       formData.batch_code !== '' ||
  ...
  }, [selectedDeliveries, formData, step, warehouseType, saveDraft])
  ...
  // Update planned week for remaining quantity
  const handlePlannedWeekChange = (deliveryId: string, weekIso: string) => {
    const newSelected = new Map(selectedDeliveries)
    const delivery = newSelected.get(deliveryId)
    if (delivery) {
      delivery.plannedWeekIso = weekIso
      newSelected.set(deliveryId, delivery)
      setSelectedDeliveries(newSelected)
    }
  }

  // Generate next 12 weeks in ISO format
  const getNext12Weeks = (): string[] => {
    const weeks: string[] = []
    const today = new Date()
    for (let i = 0; i < 12; i++) {
      const futureDate = addWeeks(today, i)
      const year = getISOWeekYear(futureDate)
      const week = getISOWeek(futureDate)
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
              <div className="mb-4 flex items-center">
                ...
                <h3 className="text-lg font-semibold">检测到未完成的草稿</h3>
              </div>
                ...
                {loadDraft() && (
                  <>
                    ...
                    <span className="font-medium text-blue-600">
                      {formatSavedTime(loadDraft()!.savedAt)}
                    </span>
                    ，是否恢复草稿内容？
                  </>
                )}
              ...
              <div className="flex justify-end space-x-3">
                <Button onClick={discardDraft} variant="outline">
                  丢弃草稿
                </Button>
                <Button onClick={restoreDraft} variant="primary">
                  恢复草稿
                </Button>
              </div>
            </div>
        ...
        {draftSavedAt && (
          <div className="mb-4 flex items-center justify-between rounded-lg border border-blue-200 bg-blue-50 px-4 py-3">
            <div className="flex items-center text-sm text-blue-700">
              ...
              <span>
                草稿已保存 ({formatSavedTime(draftSavedAt)})
              </span>
            </div>
            <Button
              onClick={clearDraft}
                ...
                {selectedArray.map((delivery) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Planned Week Input for Remaining Quantity */}
                      {delivery.userShippedQty < delivery.unshipped_qty && (
                        <div className="col-span-full mt-3 p-3 bg-amber-50 rounded-lg border border-amber-200">
                          <div className="flex items-center gap-2 text-amber-700 mb-2">
                            <AlertCircle className="h-4 w-4" />
                            <span className="font-medium">
                              剩余 {delivery.unshipped_qty - delivery.userShippedQty} 件未发运
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
                            <Label htmlFor={`planned-${delivery.delivery_id}`} className="flex items-center gap-1">
                              <Calendar className="h-4 w-4" />
                              预计发运周:
                            </Label>
                            <select
                              id={`planned-${delivery.delivery_id}`}
                              value={delivery.plannedWeekIso || ''}
                              onChange={(e) => handlePlannedWeekChange(delivery.delivery_id, e.target.value)}
                              className="flex h-9 w-32 appearance-none rounded-lg border border-amber-300 bg-white px-3 py-1 text-sm ring-offset-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500 focus-visible:ring-offset-2"
                            >
                              <option value="">选择周次</option>
                              {getNext12Weeks().map(week => (
                                <option key={week} value={week}>{week}</option>
                              ))}
                            </select>
                            {delivery.plannedWeekIso && (
