...
import { AlgorithmAuditV3Filters } from '@/components/inventory/algorithm-audit-v3-filters'
...
export default async function AlgorithmAuditV3Page({ searchParams }: PageProps) {
  ...
  if (!selectedSku) {
    return (
      <div className="p-8">
        <h1 className="text-2xl font-semibold mb-6">算法验证 V3.0 Algorithm Audit</h1>
        <Card>
  ...
  if (!auditData.product) {
    redirect('/inventory/algorithm-audit-v3')
  }
  ...
  return (
    <div className="p-8 space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-semibold mb-2">算法验证 V3.0 Algorithm Audit</h1>
      ...
      <Card>
        ...
        <CardContent>
          <Suspense fallback={<div className="p-12 text-center text-gray-500">加载中...</div>}>
            <AlgorithmAuditTableV3 rows={rows} />
          </Suspense>
        </CardContent>
      </Card>
    </div>
